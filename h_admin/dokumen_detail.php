...
//   $d = date_create($tgl_jual);
//   $tgl_jual = date_format($d, "Y-m-d");
//   $keterangan     = $_POST['keterangan'];
if (!empty($_FILES)) {
    if ($size < 1000000000) {
      $video = $path . $baru;
      if (move_uploaded_file($tmp_name, $video)) {
        $query = mysqli_query($connect, "INSERT INTO transaksi_penjualan (id_penjualan, id_cust, id_barang, keterangan ,tgl_jual, total_harga) 
													VALUES ('$id_penjualan','$id_cust','$id_barang','$keterangan','$tgl_jual','$total_harga')");
        if ($query) {
...
if(isset($_POST['ganti'])){
		$id_file    =$_POST['id_penjualan'];
		$nama_pembeli    =$_POST['nama_pembeli'];
		// $tgl_dokumen   =$_POST['tgl_dokumen'];
		// $d = date_create($tgl_dokumen);
		// $tgl_dokumen = date_format($d, "Y-m-d");
		$keterangan = $_POST['keterangan'];
		// $keterangan    =$_POST['keterangan'];

		$query3 = "UPDATE transaksi_penjualan set nama_pembeli='$nama_pembeli', keterangan='$keterangan' WHERE id_penjualan='$id_penjualan'"; 
	...
	if(isset($_POST['on'])){
		...
		$query3 = "UPDATE transaksi_penjualan WHERE id_penjualan='$id_penjualan'"; 
										...
										<div style="overflow: auto;">
											<table id="dynamic-table" class="table table-striped table-bordered table-hover">
												...
												<tbody>
													...
													$query = "SELECT * FROM transaksi_penjualan A INNER JOIN tb_customer B 
																ON A.keterangan, tgl_jual, total_harga=B.nama, alamat, telepon WHERE id_penjualan='$id_p'";
